...
func Update(ctx *gin.Context) {
	...
	// 2.再修改
	if tx := mysql.Instance().Model(new(structure.Participle)).Where("id = ?", req.ID).UpdateColumn("status", req.Status); tx.Error != nil {
		trace, response := define.Response(define.CodeSvrInternalError, nil)
		logger.App().Errorf("Update error: [%s]-%s", trace, tx.Error.Error())
		ctx.AbortWithStatusJSON(http.StatusOK, response)
		return
	...
	// 3.查出来
	participle := &structure.Participle{}
	if tx := mysql.Instance().Model(participle).Where("id = ?", req.ID).First(participle); tx.Error != nil {
		trace, response := define.Response(define.CodeSvrInternalError, nil)
		logger.App().Errorf("Update error: [%s]-%s", trace, tx.Error.Error())
		ctx.AbortWithStatusJSON(http.StatusOK, response)
		return
