...
func Delete(ctx *gin.Context) {
	...
	// 1.先查询有没有
	exist := int64(0)
	if tx := mysql.Instance().Model(new(structure.TGSpiderAccount)).Where("id = ?", req.ID).Count(&exist); tx.Error != nil && !errors.Is(tx.Error, gorm.ErrRecordNotFound) {
	...
	// 2.再修改
	if tx := mysql.Instance().Model(new(structure.TGSpiderAccount)).Where("id = ?", req.ID).Delete(new(structure.TGSpiderAccount)); tx.Error != nil {
		trace, response := define.Response(define.CodeSvrInternalError, nil)
		logger.App().Errorf("Delete error: [%s]-%s", trace, tx.Error.Error())
		ctx.AbortWithStatusJSON(http.StatusOK, response)
		return
