...
type QueryRsp struct {
	...
	List     []structure.TGSpiderAccount `json:"list"`
}
...
func Query(ctx *gin.Context) {
	...
	tx := mysql.Instance().Model(new(structure.TGSpiderAccount))

	if req.Phone != "" {
		tx = tx.Where(fmt.Sprintf("phone LIKE'%%%s%%'", req.Phone))
	}
	if req.Status != 0 {
	...
	list := make([]structure.TGSpiderAccount, 0)
